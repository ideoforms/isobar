...
class MidiFileInputDevice:
    ...
    def read(self, quantize=None):
        ...
        log.info(
            ...
            % (self.filename, midi_reader.ticks_per_beat)
        )
        note_tracks = list(
            filter(
                lambda track: any(message.type == "note_on" for message in track),
                midi_reader.tracks,
            )
        )
        if not note_tracks:
        ...
        for event in track:
            if event.type == "note_on" and (event.velocity > 0 or event.note == 0):
                ...
                if {
                    k: v for (k, v) in event.__dict__.items() if k in fake_note_off
                } == fake_note_off:
                    continue
                offset += event.time / midi_reader.ticks_per_beat
        ...
        for note in notes:
            if isinstance(note, MidiNote):
                if not note.duration:
                    note.duration = 1
                log.debug(
                    " - MIDI event (t = %.2f): Note %d, velocity %d, duration %.3f"
        ...
        for i, t in enumerate(times):
            ...
            if len(notes) > 1:
                ...
                note_dict[EVENT_AMPLITUDE].append(
                    tuple(note.velocity for note in notes)
                )
                note_dict[EVENT_GATE].append(
                    tuple(note.duration / time_until_next_note for note in notes)
                )
            else:
